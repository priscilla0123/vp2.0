...
function check(el, vm, target, rules, fieldName, tag, autocheck) {
    ...
    for (var ruleName in rules) {
        if (Rule[ruleName]) { //default rule
            ...
            var arg = Array.isArray(rules[ruleName]) ? rules[ruleName][0] : rules[ruleName];
            var msg = Array.isArray(rules[ruleName]) && rules[ruleName][1] ? rules[ruleName][1] : rule.msg;
            ...
            switch (el.localName) {
                case 'input':
                    if (el.type == 'text' || el.type == 'file')
                        return el;
                    if (el.type == 'radio' && document.getElementsByName(el.name).length) {
                        return document.getElementsByName(el.name);
                    }
                    if (el.type == 'checkbox' && document.getElementsByName(el.name).length) {
                        return document.getElementsByName(el.name);
                    }
                case 'select':
                    return el;
