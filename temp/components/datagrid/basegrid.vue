    ...
    <table class="lg-table">
        ...
        <tbody v-if="aData.length > 0">
            <template v-for="(item, i) in aData">
                <slot :name="linename(i)">
                    <tr v-line="{lineElements,i}">
                        <td v-if="expand">
                            <span class="lg-checkbox-plain">
                                <input type="checkbox" :value="'exp'+i" :id="uid('exp',i)" @click="onExpand(i)" v-model="aCheckResults.exp"/>
                        ...
                        <td v-for="col in aLeafColumn" class="nowrap">
                            <slot :name="cellname(col, i)" :title="item[col.key]">
                                <span v-if="isType('checkbox',col,item[col.key])" class="lg-checkbox">
                                    <input type="checkbox" :value="item[col.key].value" :id="uid(col,i)" @click="onCheck(col.key, i,item[col.key].value)" v-model="aCheckResults[col.key]" v-checked="{key:col.key,item,type:'checkbox'}"/>
                                    <label :for="uid(col,i)"></label>
                                </span>
                                <span v-if="isType('radio',col,item[col.key])" class="lg-radio">
                                    <input type="radio" :name="uid(col)" :id="uid(col,i)" v-model="aCheckResults[col.key]" @click="onRadio(col.key,i,item[col.key].value)" :value="item[col.key].value" v-checked="{key:col.key,item,type:'radio'}"/><label v-html="item[col.key].label" :for="uid(col,i)"></label> 
                                </span>
...
var BaseGrid = {
            ...
            default: function(data, key) {
                if (data[key] != undefined) {
                    if (typeof data[key] != 'function' && typeof data[key] != 'object') {
                        return data[key];
                    } else if (data[key].label || data[key].label === 0) {
    ...
    computed: {
        ...
        aCheckResults(){ 
            ...
            var result=null;
            if(this.data){
                result=JSON.parse(JSON.stringify(this.checkResults));
                this.isAllCheck=[];
            }         
            return result;
        },
    ...
    methods: {
        ...
        onCheck(key, index) {
            this.$emit('check', key, index, this.aCheckResults[key].join(','));
        ...
        onCheckAll(key) {
            ...
            var length = this.aCheckResults[key].length + disableLength;
            this.aCheckResults[key] = [];
            if (length != this.aData.length) {
                this.aData.forEach(function(line) {
                    !line[key].disable && _this.aCheckResults[key].push(line[key].value);
                })
            } else {}
            this.$emit('checkall', key, this.aCheckResults[key].join(','));
            this.computeCheckAll(key);
        },
        onRadio(key, index) {
            this.$emit('radio', key, index, this.aCheckResults[key]);
        },
        ...
        onExpandAll() {
            var _this = this;
            var length = this.aCheckResults.exp.length;
            this.aCheckResults.exp = [];
            if (length != this.aData.length) {
                this.aData.forEach(function(line, i) {
                    _this.aCheckResults.exp.push('exp' + i);
                })
            } else {}
            this.$emit('expandall');
        ...
        onExpand(index) {
            this.$emit('expand', index, this.aCheckResults.exp.join(','));
            this.computeExpandAll();
        },
        getRowHeight() {
            var result = [];
        ...
        isExpand(index) {
            return this.aCheckResults.exp.indexOf('exp' + index) >= 0;
        },
        ...
        computeCheckAll(key) {
            ...
            if (length != this.aData.length) {
                ...
                index == -1 && this.isAllCheck.push(key);
            }
        },
        computeExpandAll() {
            var length = this.aCheckResults.exp.length;
    ...
    directives: {
        ...
        checked: {
            inserted(el, { value }, vnode) {
                if (value) {
                    var key = value.key,
                        val = value.item[key].value,
                        type = value.type,
                        checked = value.item[key].checked;
                    if (checked) {
                        if(type=='radio'){
                            vnode.context.aCheckResults[key]=val;
                        }else{
                            vnode.context.aCheckResults[key].push(val);
                            vnode.context.computeCheckAll(key);
                        }
                    }
                }
            }
