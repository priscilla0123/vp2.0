...
export default {
    ...
    methods: {
        ...
        setCalendar(year, month, singleset) {
            this.calendarData = this.checkDisable(calendar(year, month - 1));
            !singleset && this.selectDate({ year, month, date: this.curDate }, true);
        },
        checkDisable(calendar) {
            calendar.forEach(item => {
                let date;
                item.forEach(it => {
                    date = new Date(it.year + '.' + it.month + '.' + it.date + ' 00:00:00');
    ...
    created() {
        ...
        this.calendarData = this.checkDisable(calendar(this.curYear, this.curMonth - 1));
        !this.hasChecked && (this.hasChecked = true);
    ...
    &-list {
        &-items {
            ...
            >div {
                ...
                &:first-child,
                &:first-child.current-month,
                &:first-child.simple-date {
                    color: #FF6E40;
                }
                &.normal-date,
                &.simple-date {
