<template>
    <div class="select" :class="{ hover: hoverSelect, focus: selectMode, disabled: disabled }" :style="{ width: +this.width + 'px' }" @mouseenter.stop="!disabled && (hoverSelect = true)" @mouseleave.stop="hoverSelect = false" v-clickoutside="outside">
        ...
        <transition name="dropDown">
            <ul class="select-list" v-show="selectMode" @mouseleave="hoverKey = undefined">
                <li v-for="option in options" @mouseenter="!option.disabled && (hoverKey = option.value)" @click="select(option)" :class="{ hover: hoverKey == option.value, active: activeKey == option.value, disabled: option.disabled }" :key="option.value">
                    <template v-if="type === 'multiple'">
                        <label>
                            <checkbox class="checkbox" v-model="val" :value="option.value" :disabled="option.disabled"></checkbox>{{ option.text }}</label>
                    </template>
        ...
        setVal(c) {
            if (this.type === 'multiple' && Array.isArray(c)) {
                ...
                this.options.forEach(item => {
                    if ((this.value !== null && this.value !== undefined && this.value !== '' && item.value == this.value) || (this.value === "" && item.value === this.value)) {
                        this.text = item ? item.text : undefined;
